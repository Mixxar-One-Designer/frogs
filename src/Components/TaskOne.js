...
const TaskOne = ({ showModal, setShowModal }) => {
  ...
  const handleTaskLinkClick = () => {
    window.open("https://t.me/plutotapofficials");
  ...
  const handleVerify = async () => {
    ...
    const response = await fetch(
      `https://api.telegram.org/bot7327868388:AAH89ye-KeNy0TcsWFe-ixgXFXKOSRwYDj8/getChatMember?chat_id=-1002227548826&user_id=${id}`
    );
